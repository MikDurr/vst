#!/usr/bin/env bash
# Launch stemsep: the Python engine (:8020) and the UI (:5190).
# Ports deliberately avoid the vocal training studio's 8000/5173 and vocalign's
# 8010/5180 so all three projects can run at once. Ctrl+C stops both.
set -euo pipefail
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$DIR"

API_PORT=8020
WEB_PORT=5190   # must match vite.config.ts and api/main.py's CORS allowlist
...
# :5190 would then accept connections but never answer one — a browser tab
...
[ -d "$DIR/web/node_modules" ] || { echo "→ installing UI deps"; ( cd "$DIR/web" && npm install ); }

if port_busy $API_PORT; then echo "✓ engine already on :$API_PORT"
else
  echo "→ starting engine on :$API_PORT"
  "$PY" -m uvicorn api.main:app --port "$API_PORT" < "$NO_STDIN" & API_PID=$!
fi
...
echo
echo "  stemsep → http://localhost:$WEB_PORT"
echo "  Ctrl+C to stop."
echo
wait
